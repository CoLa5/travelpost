...
def toc_flowables(
    ...
    match num_columns:
        case 1:
            page = TOCStartSinglePage
        case 2:
            page = TOCStartDoublePage
        case _:
            msg = f"{num_columns:d} columns is not supported"
            raise RuntimeError(msg)

    flows = [
        NextPageTemplate(page.id),
        PageBreak(),
        FrameBreak(page.title_frame_id),
        ...
        H1(title),
        FrameBreak(page.toc_frame_id),
    ]
    if num_columns == 2:
        flows.append(NextPageTemplate(TOCDoublePage.id))
    flows.extend(
        (
            TableOfContents(
                dots_min_level=-1,
